...
func (g generalRepo) Create(ctx context.Context, v any, scope ...func(tx *gorm.DB) *gorm.DB) error {
	return g.data.db.WithContext(ctx).Scopes(scope...).Create(v).Error
}
...
func (g generalRepo) GetBy(ctx context.Context, dest schema.Tabler, scope biz.Scope) error {
	return g.data.db.WithContext(ctx).Scopes(scope).First(dest).Error
}

func (g generalRepo) Find(ctx context.Context, dest any, scope biz.Scope) error {
	return g.data.db.WithContext(ctx).Scopes(scope).Find(dest).Error
}

func (g generalRepo) Count(ctx context.Context, m schema.Tabler, scope biz.Scope) (count int64, err error) {
	err = g.data.db.WithContext(ctx).Model(m).Scopes(scope).Count(&count).Error
	return
}

func (g generalRepo) FindBy(ctx context.Context, dest any, scope biz.Scope, pager biz.Pagination) (count int64, err error) {
...
func (g generalRepo) UpdateBy(ctx context.Context, m schema.Tabler, scope biz.Scope, values map[string]any) error {
...
func (g generalRepo) DeleteBy(ctx context.Context, m schema.Tabler, scope biz.Scope) error {
	return g.data.db.WithContext(ctx).Scopes(scope).Delete(m).Error
}

func NewGeneralRepo(data *Data) biz.GeneralRepo {
	return &generalRepo{
		data: data,
	}
}
